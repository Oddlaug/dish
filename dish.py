...
class CookBook(DataBaseProcessor, StringProcessor):
    ...
    def get_shop_list(self, person_count, *dishes):
        self.load_db()
...
if __name__ == "__main__":
    database = "cookbook.dat"
    export_file = "export.txt"
    my_cookbook = CookBook(database, export_file)
    # my_cookbook.create_cook_book()
    # my_cookbook.add_recipe()

    dishes = ['омлет', 'Овсяная каша', 'фарш']
    persons = 10
    my_cookbook.get_shop_list(persons, *dishes)
    my_cookbook.export_to_file()
    my_cookbook.get_recipes()
